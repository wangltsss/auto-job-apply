...
test('runExecutor records successful attempts in the application ledger', async () => {
  ...
  try {
    ...
    const ledger = JSON.parse(await readFile(ledgerStorePath, 'utf-8')) as {
      attempts: Array<{ outcome: string; application_url: string; answer_summary: Array<{ provenance: string }> }>;
      successes: Array<{ application_url: string; submitted_answers_summary: Array<{ provenance: string }> }>;
      failures: unknown[];
    ...
    expect(ledger.attempts[0]?.answer_summary[0]?.provenance).toBeTruthy();
    expect(ledger.successes).toHaveLength(1);
    expect(ledger.successes[0]?.submitted_answers_summary.length).toBeGreaterThan(0);
    expect(ledger.successes[0]?.submitted_answers_summary[0]?.provenance).toBeTruthy();
    expect(ledger.failures).toHaveLength(0);
    expect(ledger.clarifications).toHaveLength(0);
  } finally {
...
test('runExecutor records failed attempts in the application ledger', async () => {
  ...
  try {
    ...
    const ledger = JSON.parse(await readFile(ledgerStorePath, 'utf-8')) as {
      attempts: Array<{ outcome: string; failure_code: string | null; answer_summary: Array<{ provenance: string }> }>;
      successes: unknown[];
      failures: Array<{ failure_code: string | null; details: { notes: string[] } }>;
      clarifications: Array<{ provenance: string; resolved: boolean }>;
    };
    expect(ledger.attempts).toHaveLength(1);
    ...
    expect(ledger.clarifications[0]?.resolved).toBeFalsy();
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
});
